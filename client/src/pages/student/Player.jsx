...
const Player = () => {
  ...
  }, [enrolledCourses, courseId]) ;//useEffect runs when react component renders, only re-run this effect when the dependency array changes
  ...
  const handleRate = async (rating) => {
    try {
      const token = await getToken()
      const {data} = await axios.post(backendUrl + '/api/user/add-rating', {rating, courseId}, {headers: {Authorization: `Bearer ${token}`}})
